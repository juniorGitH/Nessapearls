...
const Montres = () => {
  ...
  const filters = ["Tout", "Casio", "Rolex", "Poedagar", "Hublot", "Personnalisable"];
  ...
  const loadArticles = async () => {
    try {
      ...
      if (data.length === 0) {
        setErrorMessage("Aucune montre trouvée.");
      }
    } catch (error) {
  ...
  const filterArticles = (filter) => {
    if (filter === "Tout") {
      ...
      const filtered = articles.filter((article) => {
        ...
        switch (filter) {
          case "Casio": return searchText.includes("casio");
          case "Rolex": return searchText.includes("rolex");
          case "Poedagar": return searchText.includes("poedagar");
          case "Hublot": return searchText.includes("hublot");
          case "Personnalisable": return searchText.includes("personnalisable");
          default: return true;
        }
      });
  ...
  const handleBuyWhatsApp = (article) => {
    const message = `🛒 *Commande Nessa Pearls*\n\n` +
      ...
      `Je souhaite commander cette montre. Merci de me donner les détails sur la gravure et les délais.`;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-500 max-w-3xl mx-auto">
            Une sélection de gardes-temps prestigieux, personnalisables pour marquer vos moments inoubliables.
          </p>
        </div>
