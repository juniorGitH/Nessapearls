...
const Accessoires = () => {
  ...
  const filters = ["Tout", "Gourde", "Stylo", "Homme", "Autre"];
  ...
  const filterArticles = (filter) => {
    if (filter === "Tout") {
      ...
      const filtered = articles.filter((article) => {
        ...
        switch (filter) {
          case "Gourde": return searchText.includes("gourde");
          case "Stylo": return searchText.includes("stylo");
          case "Homme": return searchText.includes("homme") || searchText.includes("monnaie");
          case "Autre": return searchText.includes("tatouage") || searchText.includes("piercing") || searchText.includes("clé");
          default: return true;
        }
      });
  ...
  const handleBuyWhatsApp = (article) => {
    const message = `🛒 *Commande Nessa Pearls*\n\n` +
      `🖋️ Accessoire: ${article.nom}\n` +
      `💰 Prix: ${formatPrice(article.prix)} FCFA\n` +
      `📝 Détails: ${article.caracteristique}\n\n` +
      `Je souhaite commander cet accessoire personnalisé. Merci de me contacter pour la gravure.`;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-full text-indigo-700 text-sm font-bold mb-4 uppercase tracking-widest">
            🖋️ PERSONNALISATION UNIQUE
          </span>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 mb-6 uppercase tracking-tighter">
            ACCESSOIRES <span className="text-indigo-600">& CADEAUX</span>
          </h1>
          <p className="text-xl text-slate-500 max-w-3xl mx-auto">
            Rendez chaque objet unique grâce à notre service de gravure et de personnalisation sur mesure.
          </p>
        </div>
